(() => {
  ...
  const init = () => {
      ...
      document.querySelectorAll('code.language-razor, code.language-cshtml').forEach(code => {
        if (code.dataset.azuntRazorHighlighted === 'true') return;
        ...
        code.innerHTML = html;
        code.dataset.azuntRazorHighlighted = 'true';
      });
  };

  window.Azunt.shells.docs = window.Azunt.shells.docs || {};
  window.Azunt.shells.docs.init = init;
